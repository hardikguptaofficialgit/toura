...
import { 
  MapPin, Star, Phone, Globe, Clock, Filter, Grid, List, Map, 
  Heart, Share2, Camera, ArrowLeft, ArrowRight, ChevronDown,
  Wifi, Car, Utensils, Hotel, Mountain, TreePine, Coffee, Shield,
  BookOpen, Award, Users, Calendar, Route, Scroll, Eye
} from 'lucide-react';
...
const SearchResults: React.FC<SearchResultsProps> = ({
  ...
  const getMonasteryDetails = (place: Place): MonasteryHeritage | null => {
    ...
    // Try to find matching monastery in our database
    const monastery = MonasteryService.searchMonasteries(place.name).find(m => 
  ...
  const handleMonasteryClick = (place: Place) => {
    ...
    if (monastery) {
      ...
      // Fallback to regular place selection
      onPlaceSelect(place);
    }
  };
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className={`text-2xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
                {searchResult.searchMeta.query}
              </h2>
              <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                {searchResult.totalCount} places found in {searchResult.searchMeta.location}
              </p>
            </div>
